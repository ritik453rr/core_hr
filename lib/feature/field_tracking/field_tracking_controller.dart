...
class FieldTrackingController extends GetxController {
  ...
  var fieldStaffList = <FieldStaffLocation>[
    FieldStaffLocation(
  ...
  List<FieldStaffLocation> get filteredStaff {
    if (selectedFilter.value == 'All') return fieldStaffList;
    return fieldStaffList.where((s) => s.status.toLowerCase() == selectedFilter.value.toLowerCase()).toList();
  }

  void refreshLocations() {
    Get.snackbar('Refreshing GPS', 'Pinging live field staff coordinates...', snackPosition: SnackPosition.TOP);
  }
}
